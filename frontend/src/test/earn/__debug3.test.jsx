...
const openSelect = async (user, label) => {
  ...
  console.log("TRIGGER", label, "disabled=", btn.disabled);
  await user.click(screen.getByRole('button', { name: label }))
  const lbs = screen.queryAllByRole("listbox"); console.log("listboxes", lbs.length, lbs.map(l=>l.getAttribute("aria-label"))); console.log(document.body.innerHTML.slice(2000,5000)); return screen.findByRole("listbox", { name: label })
}
