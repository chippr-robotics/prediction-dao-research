...
// ---- WagerTagPanel dependencies (mocked so we can render each gate state without a live wallet) ----
...
// ---- AddressInput resolution hooks (mocked like the AddressInput tag test) ----
...
describe('Wager tag surfaces — accessibility (spec 054, WCAG 2.1 AA)', () => {
  beforeEach(() => {
  ...
  it('WagerTagPanel — disconnected state has no violations', async () => {
    const { container } = renderPanel()
    expect(await axe(container)).toHaveNoViolations()
  })

  it('WagerTagPanel — below-Gold upgrade prompt has no violations', async () => {
  ...
  it('AddressInput — resolved tag affordance (preview + verified badge + report link) has no violations', async () => {
    ...
    const { container } = render(
      <AddressInput id="tag-a11y" label="Opponent" value="%chipprbots" onChange={() => {}} />,
    )
    expect(await axe(container)).toHaveNoViolations()
  })
})
