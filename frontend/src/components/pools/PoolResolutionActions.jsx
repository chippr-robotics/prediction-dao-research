...
import {
  payoutMatrixHash, payoutMatrixSum, serializeSharedProposal, parseSharedProposal, parseMatrix,
...
/**
 * PoolResolutionActions (spec 034, US1 + UX round 3) — the resolution loop, redesigned so operators and
 * members never touch a raw "claim code":
 *
 *  - The creator builds ONE row per participant (auto-seeded from the roster in rank order) — no manual
 *    "add winner" row — and enters an amount per alias. Proposing shares an annotated payout the whole
 *    group can see (medals/amounts land on the roster cards), and can be REVISED if the operator mis-keyed.
 *  - Each member's payout code is system-managed: it's derived + cached at join, copied to the creator
 *    with one tap (never shown as a scary integer), and matched automatically at claim time.
 ...
 *  - Claiming is one tap: the app fills the matrix, finds the member's row, and pays their wallet.
 *
 * Verification of the proposal against the on-chain proposalId happens in the parent (PoolPage), which
 * passes the verified `payoutByCommitment` down; this component reflects it and drives the actions.
 */
export default function PoolResolutionActions({
  ...
  // ── Member: system-managed payout code (never rendered as a raw string) ────────────────────────
  const [myCode, setMyCode] = useState(claimCode)
  const [codeCopied, setCodeCopied] = useState(false)
  useEffect(() => { if (claimCode && !myCode) setMyCode(claimCode) }, [claimCode, myCode])
  const copyMyCode = async () => {
    setNotice(null)
    try {
      const code = myCode || (await getMyClaimCode(summary.address))
      setMyCode(code)
  ...
  // ── Creator: build / revise the payout (one row per participant, no manual add) ────────────────
  ...
  // The creator's OWN row (if they joined their pool) auto-fills its payout code from the cached identity,
  // so a creator who takes part doesn't have to hand a code to themselves.
  useEffect(() => {
    if (!orderedParticipants.length) return
    const priorByCommitment = new Map((verifiedProposal?.display || []).map((d) => [String(d.commitment), d.amount]))
    setRows((rs) => {
      const pristine = rs.length === 0 || rs.every((r) => !r.code && !r.amount)
      if (!pristine && rs.length === orderedParticipants.length) return rs
      return orderedParticipants.map((p) => {
        const prior = priorByCommitment.get(String(p.commitment))
        const isSelf = claimCode && summary.myCommitment && String(p.commitment) === String(summary.myCommitment)
        return {
          commitment: p.commitment,
          label: p.label,
          code: isSelf ? claimCode : '',
          amount: prior != null ? ethers.formatUnits(BigInt(prior), decimals) : '',
        }
      })
    })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [orderedParticipants, verifiedProposal, claimCode, summary.myCommitment])
  const setRow = (i, k) => (e) => setRows((rs) => rs.map((r, j) => (j === i ? { ...r, [k]: e.target.value } : r)))

  const filled = rows.filter((r) => r.code && r.amount)
  const entries = filled.map((r) => ({ claimNullifier: r.code.trim(), amount: ethers.parseUnits(String(r.amount || '0'), decimals) }))
  const display = filled.map((r) => ({ commitment: String(r.commitment), amount: ethers.parseUnits(String(r.amount || '0'), decimals) }))
  const touched = rows.filter((r) => r.code || r.amount).length
  const complete = filled.length === touched && filled.length > 0
  const sum = complete ? payoutMatrixSum(entries) : 0n
  const sumOk = complete && sum === escrow
  ...
  const propose = async () => {
    ...
    try {
      ...
      saveProposedMatrix(summary.address, proposalId, entries, display)
      await copyToClipboard(serializeSharedProposal({ entries, display }))
      setNotice(isRevision
        ? 'Updated. The revised payout is copied — share it with the group so they can review and approve.'
        : 'Proposed. The payout is copied — share it with the group so they can review and approve.')
      onChanged?.()
    } catch (e) {
      setNotice(e?.shortMessage || e?.message || String(e))
    } finally {
  ...
  // ── Member: receive the creator's shared payout (pasted once, verified in the parent) ──────────
  const [pasteText, setPasteText] = useState('')
  const receiveShared = () => {
    setNotice(null)
    const parsed = parseSharedProposal(pasteText)
    if (!parsed) { setNotice('Could not read that payout. Ask the creator to re-copy and resend it.'); return }
    if (!summary.currentProposalId || payoutMatrixHash(parsed.entries) !== summary.currentProposalId) {
      setNotice('That payout does NOT match the one proposed on-chain — do not approve based on it.')
      return
    }
    saveProposedMatrix(summary.address, summary.currentProposalId, parsed.entries, parsed.display)
    setPasteText('')
    onProposalReceived?.()
  }
  ...
  useEffect(() => {
    if (disputeOpen && disputeRows.length === 0 && orderedParticipants.length) {
      setDisputeRows(orderedParticipants.map((p) => ({ label: p.label, amount: '' })))
    }
  ...
  // ── Winner: one-tap claim (matrix + row auto-resolved; pays the connected wallet by default) ────
  const claimEntries = verifiedProposal?.entries || null
  const myAmount = payoutByCommitment && summary?.myCommitment
    ? payoutByCommitment.get(String(summary.myCommitment))
    : null
  ...
  const claim = async () => {
    ...
    try {
      ...
      const idx = myCode ? parsed.findIndex((e) => String(e.claimNullifier) === String(myCode)) : -1
      if (idx < 0) throw new Error('Could not find your row in the payout. Make sure you sent the creator your payout code.')
  ...
  return (
    <section className="pool-resolution-actions" aria-label="Resolution actions">
      {/* Member: hand your payout code to the creator (system-managed; never shown as raw text) */}
      {summary.hasJoined && summary.state === 1 && summary.isCreator !== true && (
        <div className="pool-claim-code">
          <Button variant="secondary" onClick={copyMyCode} data-testid="copy-my-code">
            {codeCopied ? 'Payout code copied ✓' : 'Copy my payout code for the creator'}
          </Button>
          <p className="pool-hint">
            This anonymous code lets the creator include you in the payout. It never reveals your wallet.
          </p>
        </div>
      )}
      ...
      {summary.isCreator && summary.state === 1 && summary.withinResolutionWindow && (
        <div className="pool-propose" data-testid="propose-builder">
          ...
          <p>
            One row per member, in your rank order. Enter each alias&apos;s amount; the total must equal the
            escrow ({ethers.formatUnits(escrow, decimals)} {summary.tokenSymbol}). Ask each winner to tap
            &ldquo;Copy my payout code&rdquo; and paste it beside their name.
          </p>
          {rows.length === 0 && <p className="pool-hint">Waiting for members to join…</p>}
          {rows.map((r, i) => (
            <div className="pool-propose-row" key={r.commitment}>
              <span className="pool-propose-nick" data-testid={`propose-nick-${i}`}>{r.label}</span>
              <input
                aria-label={`Payout code from ${r.label}`}
                placeholder={`payout code from ${r.label}`}
                value={r.code}
                onChange={setRow(i, 'code')}
              />
              <input
