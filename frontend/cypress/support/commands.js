...
Cypress.Commands.add('mockWeb3Provider', (options = {}) => {
  ...
  /*
   * `eth_getBalance` answers a FIXED 100 ETH by default, which is fine while a spec only needs the
   * connected account to look funded. It is a lie the moment a spec reads the balance of an account
   * it does not control — a recovered legacy key, say — because the stub answers 100 ETH for every
   * address on earth, including one holding nothing.
   *
   * `realBalances: true` forwards the call to the node instead, so a balance the app displays (and
   * the gas reserve it sizes from that balance) is the chain's own answer. Opt-in: the default is
   * unchanged for the specs that were written against the stub.
   */
  const realBalances = options.realBalances === true
  ...
  cy.on('window:before:load', (win) => {
     ...
     * That is what makes write txs work: a reverting eth_estimateGas now rejects, and
        ...
        return new Promise((resolve, reject) => {
          switch (method) {
            ...
            case 'eth_getBalance':
              ...
              if (realBalances) forwardToNode(method, params, resolve, reject)
