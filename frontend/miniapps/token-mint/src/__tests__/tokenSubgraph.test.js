...
import { fetchHolders, fetchActivity, SUBGRAPH_UNAVAILABLE } from '../tokenSubgraph'

const TOKEN = '0xAbC0000000000000000000000000000000000028'
// The module now takes a RESOLVED endpoint, not a chain id: a package cannot import the host's
// network config, so `host.network(chainId)?.subgraphUrl` is fed in by the caller. `null` is what
// the host reports for Mordor and Ethereum Classic.
const NO_ENDPOINT = null
const INDEXED = 'https://example.invalid/subgraph'
...
describe('no endpoint at all (Mordor, ETC)', () => {
  it('reports no-subgraph without attempting a request', async () => {
    for (const endpoint of [NO_ENDPOINT, undefined]) {
      const res = await fetchHolders(endpoint, TOKEN)
  ...
  it('does the same for the activity feed', async () => {
    const res = await fetchActivity(NO_ENDPOINT, TOKEN)
    expect(res.available).toBe(false)
    expect(res.unavailable).toBe(SUBGRAPH_UNAVAILABLE.NO_SUBGRAPH)
...
describe('an endpoint whose schema has no token entities', () => {
  it('reports not-indexed rather than throwing a GraphQL string at the member', async () => {
    ...
    const res = await fetchHolders(INDEXED, TOKEN)
  ...
  it('recognises the other wordings a GraphQL server uses for a missing field', async () => {
    for (const message of [
      ...
      expect((await fetchHolders(INDEXED, TOKEN)).unavailable).toBe(SUBGRAPH_UNAVAILABLE.NOT_INDEXED)
    }
  })

  it('keeps the server\'s own words as detail, for a developer — not for the member', async () => {
    fetchMock.mockResolvedValue(respond(SCHEMA_ERROR))
    expect((await fetchHolders(INDEXED, TOKEN)).detail).toMatch(/has no field/)
  })
})
...
describe('a transport failure is never dressed up as a limitation of the network', () => {
  ...
  it('reports unreachable when the request throws', async () => {
    ...
    expect((await fetchActivity(INDEXED, TOKEN)).unavailable).toBe(SUBGRAPH_UNAVAILABLE.UNREACHABLE)
  })
  ...
  it('reports unreachable for a GraphQL error it cannot positively identify', async () => {
    ...
    expect((await fetchHolders(INDEXED, TOKEN)).unavailable).toBe(SUBGRAPH_UNAVAILABLE.UNREACHABLE)
  })

  it('never throws — the caller must be able to branch, not catch', async () => {
    fetchMock.mockRejectedValue(new Error('boom'))
    await expect(fetchHolders(INDEXED, TOKEN)).resolves.toBeTruthy()
    await expect(fetchActivity(INDEXED, TOKEN)).resolves.toBeTruthy()
  })
})
...
describe('a subgraph that does index tokens', () => {
  ...
  it('lower-cases the token address in the query variables', async () => {
    fetchMock.mockResolvedValue(respond({ data: { holders: [] } }))
    await fetchHolders(INDEXED, TOKEN)
  ...
  it('exposes activity under both its entity name and the panel\'s alias', async () => {
    ...
    const res = await fetchActivity(INDEXED, TOKEN)
