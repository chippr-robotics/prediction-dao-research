# Runbook: Wager Tag Registry — deploy & operations (spec 054)

How to deploy and operate the **WagerTagRegistry** — the in-house `%tag` naming
...
Background: [developer-guide/wager-tags.md](../developer-guide/wager-tags.md) (design +
lifecycle). End-user behaviour: [user-guide/wager-tags.md](../user-guide/wager-tags.md).
...
> normalized `%tag` to an owner address. It holds **no funds**, has its **own**
...
npm test                          # test/wagerTagRegistry*.test.js + integration must pass
```
...
`config/reserved-tags.json`, and **appends** `wagerTagRegistry` /
`wagerTagRegistryImpl` to the existing record. It never touches the live core proxies.

```bash
# Mordor (ETC testnet)
GAS_PRICE_WEI=100000000000 npx hardhat run scripts/deploy/deploy-wager-tag-registry.js --network mordor

# Polygon mainnet
npx hardhat run scripts/deploy/deploy-wager-tag-registry.js --network polygon
```

Re-running after `wagerTagRegistry` is recorded **aborts by design** (use an upgrade to
...
```bash
npm run sync:frontend-contracts   # populates the wagerTagRegistry address in frontend config
npm run verify:<net>              # verify the impl on the explorer (network-aware)
```

Confirm the appended `wagerTagRegistry` / `wagerTagRegistryImpl` in
`deployments/<net>-chain<id>-v2.json`, and that the frontend now resolves the address
(`getContractAddressForChain('wagerTagRegistry', chainId)`). Until this sync lands, the
admin screen and all tag resolution show a "not configured on this network" state.
...
| `MODERATOR_ROLE` | Suspend / unsuspend a tag (`setSuspended`) |
...
## Operate: the AdminPanel "Wager Tags" tab

Everything below is a role-gated write from `/admin` → **Wager Tags** (component
`frontend/src/components/admin/TagRegistryAdmin.jsx`). Each control is enabled only if
...
- **Metrics** — registry-wide counts, the current suspended / verified / reserved
  ...
  **Refresh**, a short TTL cache, and an honest "recent window only" banner). For
  complete historical totals, add a subgraph `Tag` entity (future work).
- **Moderation** — look up a `%tag` → see live status/owner/verified/reserved → then:
  - **Suspend** (MODERATOR): stops the tag resolving/displaying. **Never** reassigns the
    tag or moves funds; the owner keeps ownership and all other functionality.
...
- **Impersonation / abuse report** — users file reports via the in-app **Report** link
  ...
  confirmed report:
  - **Suspend** the offending tag (MODERATOR). This is the primary lever: it stops
...
- **Verification** — only set `setVerified` after a real off-chain identity review.
  Remember a verified badge is cleared automatically if the tag is ever repointed to a
  new owner (re-verify the new owner if warranted).
- **What you cannot (and must not try to) do** — there is **no code path** to move a tag
...
- The scan is a bounded backward lookback; when the window is capped it flags the tally
  as "recent window only." A subgraph `Tag` entity is the path to complete history.
...
`upgradeProxy`) — never a fresh redeploy, which would strand every registered tag. See
[contract-upgrades.md](./contract-upgrades.md). `deployments/<net>-chain<id>-v2.json`
records the proxy (`wagerTagRegistry`) and current implementation
(`wagerTagRegistryImpl`).

## Reserved-list curation

`config/reserved-tags.json` is the **initial** seed only (platform/brand names,
operational terms). Terms must already be canonical (3–20 chars, lowercase `a–z0–9`,
single interior hyphens) or they can't collide with a registrable tag. Curate additively
...
| Deploy aborts "already recorded" | `wagerTagRegistry` is already in the record — use an upgrade, not this script. |
...
- Design: [developer-guide/wager-tags.md](../developer-guide/wager-tags.md)
- End users: [user-guide/wager-tags.md](../user-guide/wager-tags.md)
...
- Spec: `specs/054-wager-tag-registry/`
