...
| `CHALLENGE_SECRET` (Turnstile, spec 105) | gateway | optional | on exposure, or with a Turnstile widget rotation | The `human` tier stops resolving: verifies answer `unverifiable`, callers keep reading at the **anonymous** ceiling — throughput, never access. Rotate the widget sitekey (`VITE_CHALLENGE_SITEKEY`, a rebuild) and this secret **together**; they are a pair at Cloudflare. Boot REFUSES the published test secrets in production. |
| `RPC_ACCESS_SIGNING_KEY` + `_KID` (spec 106) | gateway | optional | **by succession, routinely** | KEY MATERIAL (spec 097 rule 3). Rotation is succession, never cutover: register the successor **public** key at the provider under a NEW kid → flip `RPC_ACCESS_SIGNING_KID` (+ key) on the gateway → retire the predecessor kid at the provider after the longest issued TTL. Clients renew on their own; no rebuild, no member-visible interruption. Getting the order wrong strands every outstanding issued credential at once. |
| `RPC_ACCESS_ADMIN_KEY` (spec 106) | gateway | optional | on exposure | The FR-026 enforcement check goes `unverifiable`, and unverifiable REFUSES issuance (the one deliberate inversion): clients fall back to public capacity, disclosed as degraded. Server-side reads are untouched. This key can read **live endpoint URL tokens in plaintext** from the provider's admin API — treat exposure as exposure of those too. |
