...
**Transacting** is the conjunction of those two steps, and
`config/passkeySupport.js#isPasskeySupported` is the single gate that evaluates it — a bundler with
no factory fails at account creation, a factory with no bundler fails at submit, and it names which
half is missing. `capabilities.passkeyAccounts` sees only the config half (networks.js deliberately
does not import contracts.js), so a surface that asks "can a transaction be sent here" must consult
`isPasskeySupported`, not the capability flag. `smartAccount.js#requirePasskeySupport` re-checks at
the transaction boundary.

⚠️ **This gate must never be applied to sign-in.** Logging in is device-scoped and chain-independent
(see [Sign-in is independent of the relayer, the bundler, and the chain](#sign-in-is-independent-of-the-relayer-the-bundler-and-the-chain)).
`useConnectorAvailability` deliberately does not call it; a regression test asserts it is never
even invoked from there, because reinstating that call is exactly what locked members out.
