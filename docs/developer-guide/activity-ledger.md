...
- **`frontend/src/data/ledger/ledgerRepository.js`** — assembles sources for
  ...
  of failing the ledger. When every **network-backed** source fails and nothing
  was collected, the result carries `readState: 'unreadable'` — the empty entry
  list is then silence, not an empty history, and no consumer may render it as
  "nothing here yet" (#1280). Sources declare `backing: 'network' | 'client'`;
  a client-store read fulfils during any outage, so it cannot testify that a
  chain went unread.
