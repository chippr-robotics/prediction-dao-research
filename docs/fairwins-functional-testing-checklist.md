...
| [ ] | DSH-15 | Pending Acceptance filter surfaces expired offers | 1. With expired offers present 2. Set Status filter to "Pending Acceptance (incl. expired)" | Expired rows appear; Time Left reads "Expired" (not the resolve-deadline countdown); row carries a Clear action |  |
| [ ] | DSH-16 | Clear expired offer (local dismiss) | 1. View an expired offer 2. Click "Clear" (or "Reclaim & Clear" as creator) | Row disappears; dismissed id persisted under `mywagers_dismissed:<account>` so it stays hidden across reloads; creator variant claims the refund FIRST and only dismisses on a confirmed transaction — a failed, cancelled or not-yet-confirmed reclaim keeps the row and says why |  |
| [ ] | DSH-16b | Expired offer reached from the activity feed | 1. As the creator of an offer nobody accepted, open the "reclaim your stake" entry in the activity feed | My Wagers opens on that wager's detail view (not an empty list) with a Claim Refund action; the hint says the stake is still held by the contract |  |
