...
| `GET /v1/bridge/:chainId/{quote, status}` | Cross-chain bridge proxy (spec 067): Across `suggested-fees` quotes and deposit status for the Transfer → Bridge surface. Read-only — the member's wallet submits the deposit itself, so the gateway holds nothing and signs nothing. `:chainId` is the **origin** chain; both endpoints of a route must be in `BRIDGE_CHAIN_IDS` (`404 unsupported_chain` otherwise — Bitcoin's string network ids and ETC/Mordor are rejected here) and a destination equal to the origin is `400 invalid_route`. Same origin-lock/killswitch; fails closed (`503 bridge_disabled`) unless `BRIDGE_ENABLED=true`, which is how the SPA hides the Bridge tab for a stated reason. **A quote is never served stale** (`503 upstream_unavailable` instead) because the quoted figure is a hard ceiling; a deposit *status* may be served stale-marked because it can only lag. An unreadable upstream body is `502 upstream_malformed` — never a synthesized quote. |
...
| `BRIDGE_ENABLED` | `false` *(= `/v1/bridge/*` fails closed)* | Enables the Across bridge proxy (spec 067). Off means `503 bridge_disabled` and the SPA hides the Bridge tab. |
| `BRIDGE_API_URL` / `BRIDGE_TIMEOUT_MS` / `BRIDGE_RETRIES` | `https://app.across.to/api` / `5000` / `1` | Upstream endpoint + per-request bounds. All routes are reads, so retries are safe. Validated only when enabled. |
| `BRIDGE_CHAIN_IDS` | `1,10,137,8453,42161` | Chains bridging is offered between (research R8 — each carries an Across SpokePool). Fewer than two distinct ids while enabled fails boot. |
| `BRIDGE_QUOTE_TTL_MS` / `BRIDGE_STATUS_TTL_MS` | `10000` / `15000` | Quote cache is **single-flight de-dup only** (a stale quote is refused, not served); status may be served stale-marked. |
| `BRIDGE_QUOTA_PER_IP` / `BRIDGE_QUOTA_GLOBAL` / `BRIDGE_QUOTA_WINDOW_MS` | `60` / `300` / `60000` | Read quotas keyed per caller IP (no signature on a GET), with the global backstop. |
| `BRIDGE_KILLSWITCH` | `false` | Ops kill for `/v1/bridge/*` only (`503 bridge_killed`), independent of the global switch. |
