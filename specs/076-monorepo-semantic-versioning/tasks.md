...
- [ ] T001 Create `scripts/release/` with a `README.md` stating it is the single authority for version computation (research R2) and that nothing else may compute a version
- [ ] T002 [P] Add `release:version`, `release:classify`, `release:artifacts` and `test:release` scripts to root `package.json`, with `test:release` running `node --test scripts/release/__tests__/`
- [ ] T003 [P] Create `CHANGELOG.md` at the repository root with a header declaring it generated by the release process and never hand-edited (FR-037)
...
- [ ] T004 Implement `scripts/release/classify.js` parsing a subject line into `{type, scope, breaking, bump}` per `contracts/version-scheme.md` §1–§2, returning a structured rejection (not a throw) for an unrecognized type so callers can render the accepted-values message
- [ ] T005 Implement the byte-gate escalation rule in `scripts/release/classify.js`: given the changed-file list, reject `chore`/`docs`/`style` when `specs/075-monorepo-workspaces/baseline-bytecode.json` or `baseline-miniapp-builds.json` is modified (`contracts/version-scheme.md` §4, research R4)
- [ ] T006 [P] Write `scripts/release/__tests__/classify.test.js` covering: every type in the bump map; `!` and `BREAKING CHANGE:` footer both yielding major; Dependabot's exact title `chore(deps): Bump X from A to B`; unknown type rejected with accepted values listed; each baseline-file escalation, and the case where a baseline file is touched by an already-sufficient type (must pass)
- [ ] T007 Implement `scripts/release/version.js`: read tags, select the highest by semver order, compute the next version from the aggregated classifications of the commit range, expose `--explain` printing each commit's classification and which one determined the bump
- [ ] T008 Implement the no-previous-release case in `scripts/release/version.js` — with zero tags the next version is the constant `v1.0.0`, never read from any `package.json` (FR-006, research R9)
- [ ] T009 Implement release-candidate numbering in `scripts/release/version.js`: `v<base>-rc.<n>` where `base` is recomputed per `staging` merge and `n` increments since the last release (FR-002, data-model.md § ReleaseCandidate)
- [ ] T010 [P] Write `scripts/release/__tests__/version.test.js` covering: **aggregation — one `feat` among many `chore`s yields minor, not patch** (FR-013, the rule most likely to regress); zero-tags yields `v1.0.0`; monotonicity (never emits ≤ an existing tag); `rc` base moving from `1.3.1-rc.1` to `1.4.0-rc.2` when a `feat` lands; empty commit range yields no version (FR-021)
- [ ] T011 Verify determinism: `node scripts/release/version.js --explain` twice on the same tree produces byte-identical output (quickstart S1)
