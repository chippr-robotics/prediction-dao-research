...
- [x] No implementation details (languages, frameworks, APIs) **in the requirements, success
      criteria, or user stories**. Named tools appear only in Problem and Assumptions, where the
      existing toolchain is a stated constraint on the work rather than a design choice this spec
      is making — see iteration 1 below.
...
- [x] No implementation details leak into the requirements (same narrowed scope as the Content
      Quality item above)
