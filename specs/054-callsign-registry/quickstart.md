# Quickstart Validation: Callsign Naming Registry (spec 054)

Runnable scenarios proving the feature end-to-end. Interfaces:
[contracts/callsign-registry-interface.md](./contracts/callsign-registry-interface.md);
...
```bash
npx hardhat test test/callsignRegistry.test.js test/callsignRegistry.intents.test.js
npm run check:storage-layout        # must include the new callsignRegistry pair
...
- **Eligibility & optionality (US1/FR-001/FR-001a)**: exhaustively drive the tier gate —
  ...
  full wager create/accept, pool join, transfer, and address-book add with no callsign required
  (SC-011), and that a Gold member who lets their tier lapse keeps the callsign through grace but
  is refused a *new* register/change until Gold again while repoint still works (US4 edge).
- **Resolve (US2)**: `resolve` returns owner + ACTIVE; unknown callsign → NONE (never a
  near-match); reverse `callsignOf` matches forward (FR-008 invariant).
- **Lifecycle (US4)**: release → QUARANTINED (register by others reverts until
  `quarantinePeriod` elapses, then succeeds); `changeCallsign` inside `changeCooldown` reverts;
...
- **Moderation (US5)**: reserved-term registration reverts; suspend stops resolution
  without touching ownership; verification flag round-trips; no code path moves a callsign to a
...
#   - one owner per callsign, one callsign per owner, forward==reverse,
...
- `lib/callsigns/normalizeCallsign` — normalization/validation table tests (mirrors on-chain rules,
  including `%` stripping, casing, hyphen edge cases).
- `useCallsign` + `useOpponentName` — priority chain: book > callsign > ENS > generated
  (FR-014); registry error → chain falls through (FR-013).
- `AddressInput` — `%callsign` entry resolves and shows full address + badge; non-ACTIVE
  statuses are not committable (FR-011/FR-022); raw address entry unaffected and never
  required (FR-009).
- `CallsignPanel` — register/change/release/repoint flows with windows surfaced; the panel is
...
npx hardhat run scripts/deploy/deploy-callsign-registry.js --network localhost
...
4. Release the callsign from A → wallet B registering `%testbot` fails ("quarantined").
