# Specification Quality Checklist: Callsign Naming Registry
...
- Ambiguities in the original request (pricing, transferability, protection windows,
  ...
  90-day quarantine / 30-day cooldown / one-callsign-per-account defaults during
...
- Whether the registry lives on-chain or off-chain is deliberately not specified here;
  ...
  either way given callsigns route value-bearing actions). Resolved in planning: on-chain
  UUPS registry.
...
- **2026-07-12 decision (product owner)**: callsigns are **strictly optional / opt-in** —
