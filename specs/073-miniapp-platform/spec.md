...
- **FR-004**: Lifecycle actions (approve, reject, suspend, deprecate) MUST be restricted on-chain to a curator role held by a multisig or governance account; vendor-of-record is the only non-platform account able to update its own app's submission fields. The curator role MUST NOT be reachable by the account that administers configuration — an administrator able to grant itself curation makes the trust boundary decorative.
- **FR-004a**: Approving or rejecting a package MUST be bound to the exact package the reviewer saw: the action names the reviewed manifest hash and MUST fail if the record no longer holds it. Naming only the app would let a vendor substitute a different package after review — while approval calldata sits publicly awaiting multisig signatures, or by front-running the approving transaction — and have unreviewed code marked Approved.
- **FR-004b**: Curators MUST be able to reject a proposed package without suspending or retiring the app, leaving any previously approved package serving. Without a reject path a refused package stays queued and is promoted by the next approval.
...
- **FR-010**: On every launch request the host MUST re-verify from the registry (or a registry-derived index whose staleness is bounded and disclosed) that the app is **launchable** before any package code executes. Launchable means the record holds a curator-approved package and is neither suspended nor deprecated; the registry MUST expose this decision directly rather than leaving each host to infer it. A record whose vendor has an update in review is still launchable on its previously approved package (FR-003) — treating "an update is pending" as "not launchable" would let any vendor take their own live app offline at will.
