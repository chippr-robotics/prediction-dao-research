...
- [ ] T001 Add a `polymarket` config block to `services/relay-gateway/src/config/index.js`
...
- [ ] T002 [P] Document the new `POLYMARKET_*` env vars in
...
- [ ] T003 [P] Add `PREDICT_CHAIN_IDS = new Set([137])` and a `capabilities.predict` getter
...
- [ ] T004 [P] Create `services/relay-gateway/src/polymarket/client.js`: CLOB REST client
...
- [ ] T005 [P] Create `services/relay-gateway/src/polymarket/builderCode.js` — the
...
- [ ] T006 [P] Create `services/relay-gateway/src/polymarket/normalize.js` with DTO mappers
...
- [ ] T007 [P] Create `services/relay-gateway/src/polymarket/cache.js` — TTL cache +
  single-flight + serve-stale for reads (reuse the `opensea/cache.js` shape); order/cancel
  writes bypass it
- [ ] T008 Add the route skeleton to `services/relay-gateway/src/polymarket/routes.js`
...
- [ ] T009 Foundational gateway tests in `services/relay-gateway/test/polymarket.test.js`:
...
- [ ] T010 [P] Create the CLOB V2 order builder `frontend/src/lib/predict/clobOrder.js` per
...
- [ ] T011 [P] Unit-test `clobOrder.js` in `frontend/src/test/predict/clobOrder.test.js`:
...
- [ ] T012 [P] Create `frontend/src/lib/predict/predictClient.js` (extends the gateway-client
...
- [ ] T013 [US1] Implement the read routes in `services/relay-gateway/src/polymarket/routes.js`:
...
- [ ] T014 [US1] Gateway tests for the read + buy routes in
...
- [ ] T015 [US1] Create `frontend/src/hooks/usePredictTrade.js` — the TradeActionState machine
...
- [ ] T016 [US1] Create `frontend/src/components/predict/PredictPanel.jsx` + CSS: market
...
- [ ] T017 [US1] Create `frontend/src/components/predict/MarketDetailSheet.jsx` + CSS and
...
- [ ] T018 [US1] Tests for `TradeConfirm.jsx` in `frontend/src/test/predict/TradeConfirm.test.jsx`:
...
- [ ] T019 [US1] Wire `PredictPanel` into `frontend/src/pages/WalletPage.jsx` (render when
