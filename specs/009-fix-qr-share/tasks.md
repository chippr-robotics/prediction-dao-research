...
- The Dashboard **"Scan QR Code"** quick action opens `QRScanner`, whose **camera-error state renders only a bare `⚠️` on a pure-black modal** (`.qr-scanner-modal { background:#000 }`, `.scanner-error .error-icon { font-size:3rem }`, close `✕`). This matches the original screenshot 1 far better than a broken QR image. **The error message is in `aria-label` only — never shown as visible text.** This is a separate, un-speced UX defect (scanner failure shows a cryptic triangle). Recommend a follow-up: render the QRScanner error message as visible text + a retry/permissions hint.
