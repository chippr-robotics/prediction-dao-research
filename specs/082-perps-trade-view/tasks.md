...
- [ ] T020 [US1] `frontend/src/config/perps.js`: venue registry (labels, chains,
      non-EVM hyperliquid guard `isEvmPerpVenue`), `perpsAvailable()`, `perpsPath()`,
      cohort (mainnet-only) check
- [ ] T021 [P] [US1] `lib/perps/perpsClient.js` (PerpsUnavailable, 12s abort),
      `lib/perps/format.js` (total functions), `lib/perps/perpsCopy.js`
      (PERPS_TIPS/DISCLOSURE/risk)
- [ ] T022 [US1] `hooks/usePerpsMarkets.js`: 3-state + per-venue sources, search/
      venue-filter/sort, race-safe
- [ ] T023 [US1] `components/perps/{PerpsView,PerpsPairTable,PerpsVenueBadge}.jsx` +
      `Perps.css`: merged table, badges, degraded/unavailable/testnet notices,
      InfoTips, a11y announcements
- [ ] T024 [US1] `TradePanel.jsx` view switcher (`?view=perps`), swap view untouched
- [ ] T025 [P] [US1] Tests `frontend/src/test/perps/`: format, client, markets hook,
      view states, axe (light+dark)

## Phase 4 — US2 positions (P2)

- [ ] T030 [US2] `hooks/usePerpsPositions.js`: gateway positions read, 60s poll,
      stale-guard, account-switch hard reset, per-venue isolation
- [ ] T031 [US2] `components/perps/PerpsPositions.jsx`: read-only positions with
      venue attribution, unreadable/empty states, link-out per position
- [ ] T032 [P] [US2] Tests: positions hook + component (isolation, reset, empty)

## Phase 5 — US3 fees & admin (P2)

- [ ] T040 [US3] `scripts/deploy/lib/feeServices.js` launch entry
      `perps.hyperliquid.builder` cap 10 ConfigOnly + table test
- [ ] T041 [US3] `lib/fees/feeQuote.js` FEE_SERVICES.PERPS_HL_BUILDER;
      `FeesTab.jsx` KNOWN_SERVICES label
- [ ] T042 [US3] Gateway fee read: `fees/onchain.js` service id + `/v1/perps/config`
      live-vs-fallback (done in T012/T013)
- [ ] T043 [P] [US3] Disclosure UI: HL builder-fee line on link-out surface
      (zero ⇒ absent; unreadable ⇒ "could not be confirmed") + tests

## Phase 6 — US4 link-outs (P3)

- [ ] T050 [US4] `lib/perps/linkouts.js`: per-venue URL builders with attribution +
      plain fallback; external marking + risk disclosure in PerpsView; tests

## Phase 7 — Reporting & polish

- [ ] T060 [P] `data/notifications/sources/perpsSource.js` snapshot-diff + register;
      tests
- [ ] T061 [P] Cypress `frontend/cypress/e2e/fast/23-perps.cy.js` (stubbed gateway:
