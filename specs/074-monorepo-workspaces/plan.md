...
| **New dev dependency: `eslint-plugin-boundaries`** | FR-044–FR-046 require the spec-073 package/host boundary — a **security** boundary, since packages are untrusted third-party code — to be machine-enforced in both directions, including by package name. | **The existing Vitest guard alone**: its own docblock states a scoped run cannot catch a stale import, and it checks only relative paths, so the new by-name leak that workspaces introduce would go undetected. **Manifests alone**: verified false (R7) — relative imports never consult `package.json`, and workspaces hoist. |
