...
- [ ] T011 Extend `contracts/upgradeable/SignerIntentBase.sol` signature verification from ECDSA-only `digest.recover(sig)` to OpenZeppelin `SignatureChecker.isValidSignatureNow(signer, digest, sig)` (ECDSA first, ERC-1271 fallback for contract signers) so passkey smart accounts can be intent signers — logic-only, **no storage changes** (ERC-7201 nonce layout untouched); intent struct typehashes unchanged (three-way byte-identical rule: contracts ↔ `frontend/src/lib/relay/intentTypes.js` ↔ `services/relay-gateway/src/intent/intentTypes.js`) (analysis C1)
- [ ] T012 [P] Tests `test/intent/SignerIntentBase.erc1271.test.js`: contract-account signer accepted via ERC-1271, EOA path byte-for-byte unchanged, malicious/invalid `isValidSignature` rejected, replay/nonce/validity-window semantics preserved (extends merged `test/upgradeable/SignerIntentBase.test.js` coverage)
- [ ] T013 Ship the ERC-1271 change as in-place upgrades via the `scripts/deploy/upgrade-gasless-intents.js` pattern: both registry facets (`WagerRegistry` + `WagerRegistryIntents`, storage from `WagerRegistryCore`) and `membershipManagerImpl`; publish a new `poolImpl` version so **future** `WagerPool` clones accept ERC-1271 `…WithSig` intents (existing clones are immutable and stay ECDSA-only — documented limitation); run `npm run check:storage-layout` (gating)
- [ ] T014 Extend `services/relay-gateway/src/intent/verify.js` with an ERC-1271 fallback (when ECDSA recovery mismatches a claimed signer that has code on the bound chain, `eth_call isValidSignature(digest, sig)` against the signer) + tests in `services/relay-gateway/test/gateway.test.js` covering accept/reject/no-code paths (analysis C1, gateway leg)
- [ ] T015 [P] Fork test `test/fork/usdc-erc1271-authorization.test.js`: native USDC on Polygon accepts an EIP-3009 `receiveWithAuthorization` signed by a smart account via ERC-1271 (ERC-7598 path) — verifies the spec's stablecoin assumption (analysis U1)
