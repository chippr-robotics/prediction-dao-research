...
- **FR-003**: Sessions MUST persist across page navigation and reloads
  without re-prompting, and an explicit sign-out MUST clear all locally
  persisted session, account, and cached-role state for the active account.
...
- **FR-012**: Platform features that derive per-user encryption keys from
  ...
  any third party; where a user's authenticator cannot support this, the
  affected encrypted features MUST degrade explicitly and safely.
...
- **FR-014**: A passkey user MUST be able to complete all supported actions
  ...
  say so and present the user's actual options rather than fail opaquely.
...
- **FR-019**: Users MUST be able to add additional passkeys (including
  ...
  existing controller and take effect on-chain.
...
- **FR-023**: Where feasible, a user's passkey account SHOULD have the same
  address on the supported mainnet and testnet pair, and account state
  (roles, balances, history) MUST remain strictly scoped per network.
...
- **SC-003**: 100% of product actions available to classic-wallet users on
  ...
  the existing end-to-end product test matrix under a passkey session.
