...
- [ ] T005 [P] Implement `frontend/src/lib/pools/channel/roomSecret.js` — HKDF-SHA256 derivation of `{appId, roomId, password}` from phrase entropy (sourced from the pool summary's on-chain `wordIndices`) + pool address + chainId per contracts/handshake-payload.md Layer 1 (note: descriptor is scoping, NOT access control — inputs are public on-chain); unit tests in `frontend/src/test/poolChannelRoomSecret.test.js` (determinism, domain separation, phrase-entropy-not-rendered-words input, cross-pool/cross-chain divergence)
- [ ] T006 [P] Implement `frontend/src/lib/pools/channel/sessionAuth.js` — `fwpc-hs/1` auth message build/parse/verify (member Semaphore-identity path, creator EIP-191 path, fixed signed-tuple byte layout, nonce single-use per attempt, 10 s timeout policy, **app-session-lived certified keypair reused across reconnects — one wallet prompt per app session, never per reconnect**) per contracts/handshake-payload.md Layer 2; unit tests in `frontend/src/test/poolChannelSessionAuth.test.js` covering every failure row of the contract's table plus key-reuse-with-fresh-nonce
...
- [ ] T009 [P] Implement `frontend/src/lib/pools/channel/channelState.js` — single-writer versioned StandingsDoc + announcements list + presence doc, snapshot build/apply, latest-wins merge, bounded sizes, localStorage display-cache read/write with staleness labeling (pattern of `identityCache.js`), **creator-side persistence of authored docs + version counters with monotonic resume after reload** (data-model `…_authored_v1_…` key); unit tests in `frontend/src/test/poolChannelState.test.js` incl. reload-resume never restarting versions
...
- [ ] T012 Implement `frontend/src/lib/pools/channel/creatorHub.js` — ≤50 authenticated sessions with `bye{capacity}` decline, **≤10 pending-auth connections (camper containment, FR-020a)**, per-commitment newest-wins dedupe, on-chain member-set verification hook, **single-active-hub tab leadership via `BroadcastChannel` with `bye{superseded-hub}` on handover**, independent per-session send queues, fan-out, claim-code table keyed by commitment; and `frontend/src/lib/pools/channel/memberClient.js` — single session to creator, auth, snapshot intake, auto-reconnect with backoff + honest status transitions
- [ ] T013 Integration tests `frontend/src/test/poolChannelHub.test.js` over the fake rendezvous: full auth handshake both roles, capacity 51 decline, pending-auth cap + camper never receives channel data, commitment dedupe/supersede, hub-tab supersession, auth-timeout drop, flood → drop confined to one member, reconnect (reused session key, fresh nonce) → snapshot convergence
...
- [ ] T015 Implement `frontend/src/components/pools/channel/ChannelConnectPanel.jsx` — consent dialog (IP/rendezvous-metadata + STUN disclosure **including that the pool's rendezvous is publicly discoverable and a non-member could observe your network address until dropped at auth** — FR-021/FR-020a, decline path, persisted per data-model.md keys), connect/disconnect, truthful status ("reconnecting…", "signaling unreachable", active strategy), WCAG 2.1 AA; axe coverage started in `frontend/src/test/poolChannel.axe.test.jsx`
...
- [ ] T029 [P] Negative/posture sweep from quickstart.md §2: non-member auth rejection (SC-003), camper containment — non-member with the (publicly derivable) room descriptor never reads channel data, is dropped at auth timeout, and cannot exhaust hub capacity (FR-020a), cross-pool room isolation, both-strategies-down degradation (FR-023), signaling-blindness capture check (SC-004), flood confinement (FR-024), channel-never-connected full pool lifecycle (SC-006)
