...
- ✅ **Harness unblocked + US1 partially verified (live run, chain 1337):** the dev-warning modal/banner overlays were covering interactive elements and failing even existing specs — `mockWeb3Provider` now seeds the suppression localStorage keys (wallet-connection 0→5 passing). **T005 `19-paused-protocol.cy.js`: PAU-01 ("paused blocks new wager creation — no new wager on chain") PASSES against the live loop**; PAU-02 ("create succeeds after unpause") is `it.skip`'d pending the multi-step create wizard (verify-role → approve-token → create), the same create-completion flow that the existing `07-manual-resolution` specs partially fail here.
- ⏳ **Remaining (T004 `createAndAcceptWager` + T006–T013, and US1's PAU-02):** the chief blocker is the **multi-step create/accept wizard** — once a robust `createAndAcceptWager` helper drives verify-role/approve-token/create/accept reliably, the rest become mechanical. Author + verify with `npm run node` → `npm run deploy:local` → `npm --prefix frontend run dev` (VITE_NETWORK_ID=1337) → `cypress run --browser chrome --spec ...`, iterating selector-by-selector. The `chainTx` foundation is validated against the live deployment.
