...
- [X] T023 [US2] **DONE** — the E2E backlog was not one root cause but six layers, each only visible once the one above it was fixed. L1 entry gate covering every button; L2 the auto-connect modal the acknowledgement itself triggers; L3 mockWeb3Provider injecting a raw `window.ethereum` while wagmi 3 discovers wallets over EIP-6963, so no connector was ever offered (issue #1016, PR #1017); L4 `connectWallet` opening ConnectModal and never CHOOSING a connector, then waiting 10s for a connection nobody started — 14 call sites; L5 the spec-073 relocation of wager creation to Finance ▸ Transfer ▸ Wagers, which five specs had not followed; L6 the stake field becoming an `AmountKeypad` (`role="group"`, one button per digit, **no `<input>`**), so `.type()` could never drive it. Suite went **24 → 62 passing, 34 → 0 failing**; 42 tests are `it.skip` with a per-test reason naming the decision each awaits (#1019). Two dead `[data-testid="wager-*"]` selectors that had never existed in the app were found masked behind fallback chains — that class is audited separately in #1019.
