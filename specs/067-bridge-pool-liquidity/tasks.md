...
- [X] T094 [P] [US2] Create `frontend/src/components/earn/LiquidityPoolCard.jsx` showing kind, asset/pair, protocol, network, estimated return, total supplied, and the kind-specific risk summary with InfoTips (FR-017). **Revised:** it is now a scannable ROW carrying kind/pair/protocol/network/total/return whose whole surface opens `SupplySheet`; the risk summary, the reasons behind any missing figure, and the InfoTips moved into that sheet (FR-017 as amended)
