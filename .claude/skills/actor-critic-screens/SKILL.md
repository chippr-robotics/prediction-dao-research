...
`scripts/ui/capture-protect-hardware.mjs` (spec 085), with their findings in
...
- **The shot matrix**: every scenario × {desktop 1280×900, mobile 390×844} × {light, dark}.
  Theme is `localStorage.themeMode`. A bug can hide in exactly one cell — spec 085's invisible
