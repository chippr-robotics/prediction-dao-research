...
async function createKeystore() {
  ...
  try {
    ...
    console.log('\nRemember to unmount the floppy when done: npm run floppy:unmount');
...
async function testKeystore() {
  if (!isMounted()) {
    ...
    console.error('Run: npm run floppy:mount');
    process.exit(1);
  }
...
function showInfo() {
  if (!isMounted()) {
    ...
    console.log('Run: npm run floppy:mount');
    return;
  }
...
npm scripts (recommended):
  npm run floppy:mount     - Mount floppy
  npm run floppy:unmount   - Unmount floppy
  npm run floppy:create    - Create keystore
  npm run floppy:test      - Test keystore
