 ...
 *
 * Uses the modular RBAC system (PaymentProcessor + USDC stablecoin)
...
// Tier prices in USDC (6 decimals) - matching frontend
const TIER_PRICES = {
  BRONZE: 50,    // 50 USDC
  SILVER: 100,   // 100 USDC
  GOLD: 175,     // 175 USDC
  PLATINUM: 300  // 300 USDC
};
...
async function main() {
  ...
  console.log('Using Modular RBAC (PaymentProcessor + USDC)');
  console.log('='.repeat(60));
  ...
  // Step 6: Approve USDC and execute purchase
  ...
  const allowance = await usdcContract.allowance(wallet.address, PAYMENT_PROCESSOR);
  if (allowance < priceWei) {
    console.log('Approving USDC for PaymentProcessor...');
    const approveTx = await usdcContract.approve(PAYMENT_PROCESSOR, priceWei);
  ...
  const newUscBalance = await usdcContract.balanceOf(wallet.address);
