 ...
 */
function getSeedPlayers(ethers, networkName) {
  const players = [];
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    if (privateKey) {
      ...
      if (networkName !== "localhost" && networkName !== "hardhat" && EXAMPLE_KEYS.includes(privateKey.toLowerCase())) {
        throw new Error(
          `SEED_PLAYER_${i} is using an example private key from .env.example. ` +
          `These well-known keys are insecure and must not be used on ${networkName}. ` +
 ...
 */
async function getMarketFactory(ethers, CONFIG, deployer) {
 ...
 */
function generateMarketParams(ethers, CONFIG) {
  ...
  const liquidity = randomInRange(ethers, CONFIG.minLiquidity, CONFIG.maxLiquidity);
 ...
 */
async function createMarkets(ethers, CONFIG, marketFactory, deployer) {
  console.log(`\n📊 Creating ${CONFIG.marketsPerCycle} market(s)...`);
  const createdMarkets = [];

  for (let i = 0; i < CONFIG.marketsPerCycle; i++) {
    try {
      const params = generateMarketParams(ethers, CONFIG);
 ...
 */
async function executeTrades(ethers, CONFIG, marketFactory, players) {
  console.log(`\n💱 Executing trades...`);
  ...
  for (const player of players) {
    ...
    for (let i = 0; i < tradesThisCycle; i++) {
      try {
        ...
        const amount = randomInRange(ethers, CONFIG.minTradeAmount, CONFIG.maxTradeAmount);
 ...
 */
async function checkBalances(ethers, players) {
 ...
 * Main seeding cycle
 */
async function runCycle(ethers, CONFIG, marketFactory, deployer, players) {
  cycleCount++;
  ...
  try {
    ...
    if (cycleCount % 5 === 0) {
      await checkBalances(ethers, players);
    }

    // Create new markets
    await createMarkets(ethers, CONFIG, marketFactory, deployer);

    // Execute trades
    await executeTrades(ethers, CONFIG, marketFactory, players);
 ...
 */
function randomInRange(ethers, min, max) {
...
async function main() {
  ...
  // Configuration (needs ethers for parseEther)
  const CONFIG = {
    // Timing
...
`);
  ...
  // Load seed players
  const players = getSeedPlayers(ethers, networkName);
  ...
  // Get or deploy market factory
  const marketFactory = await getMarketFactory(ethers, CONFIG, deployer);
  ...
  while (isRunning) {
    await runCycle(ethers, CONFIG, marketFactory, deployer, players);
