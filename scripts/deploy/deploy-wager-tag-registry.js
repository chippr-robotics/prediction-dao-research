/**
 * Targeted spec-054 deploy: add the WagerTagRegistry to an ALREADY-deployed network WITHOUT touching
 ...
 * reserved-term list from config/reserved-tags.json (FR-004).
 *
 ...
 *
 * Then: npm run sync:frontend-contracts  (frontend picks up the wagerTagRegistry address).
 */
...
async function main() {
  ...
  console.log("Wager Tag Registry (spec 054) — targeted deploy");
  console.log("=".repeat(60));
  ...
  if (contracts.wagerTagRegistry) {
    console.log(`\n⚠️  wagerTagRegistry already recorded (${contracts.wagerTagRegistry}). To change logic,`);
    console.log(`   run an in-place upgrade (lib/upgradeable.js upgradeProxy), not this script. Aborting.`);
    return;
  }

  console.log("\nDeploying WagerTagRegistry behind a UUPS proxy...");
  const proxy = await deployProxy({
    name: "WagerTagRegistry",
  ...
  const seed = JSON.parse(fs.readFileSync(path.join(process.cwd(), "config", "reserved-tags.json"), "utf8"));
  ...
  contracts.wagerTagRegistry = proxy.proxy;
  contracts.wagerTagRegistryImpl = proxy.implementation;
  record.constructorArgs = record.constructorArgs || {};
  record.constructorArgs.wagerTagRegistryImpl = [];
  record.wagerTagRegistryDeployedAt = new Date().toISOString();
  saveDeployment(filename, record);

  console.log("\n" + "=".repeat(60));
  console.log("Appended to deployments/" + filename);
  console.log("=".repeat(60));
  console.log(`  wagerTagRegistry     ${contracts.wagerTagRegistry}`);
  console.log(`  wagerTagRegistryImpl ${contracts.wagerTagRegistryImpl}`);
