...
const TYPES = {
  CommitCallsignIntent: [{ name: "owner", type: "address" }, { name: "commitment", type: "bytes32" }, ...TRAILING],
  RegisterCallsignIntent: [{ name: "owner", type: "address" }, { name: "callsign", type: "string" }, { name: "salt", type: "bytes32" }, ...TRAILING],
  ReleaseCallsignIntent: [{ name: "owner", type: "address" }, { name: "callsignHash", type: "bytes32" }, ...TRAILING],
  RequestRepointIntent: [{ name: "owner", type: "address" }, { name: "callsignHash", type: "bytes32" }, { name: "newOwner", type: "address" }, ...TRAILING],
};
...
async function fixture() {
  ...
  const reg = await deployProxy("CallsignRegistry", [admin.address, await membership.getAddress(), ethers.ZeroAddress, WAGER_PARTICIPANT_ROLE]);
  await membership.connect(admin).grantMembership(owner.address, WAGER_PARTICIPANT_ROLE, Tier.Gold, 365);

  const net = await ethers.provider.getNetwork();
  const domain = { name: "FairWins CallsignRegistry", version: "1", chainId: net.chainId, verifyingContract: await reg.getAddress() };
...
describe("CallsignRegistry — gasless intents", () => {
  it("relayer submits owner-signed commit + register (self-submit rail bypassed)", async () => {
    ...
    const commitSig = await sign(owner, domain, "CommitCallsignIntent", { owner: owner.address, commitment, ...window });
    ...
    const regSig = await sign(owner, domain, "RegisterCallsignIntent", { owner: owner.address, callsign: "gasless", salt: SALT, ...w2 });
    await reg.connect(relayer).registerWithSig(owner.address, "gasless", SALT, w2.nonce, w2.validAfter, w2.validBefore, regSig);

    expect(await reg.callsignOf(owner.address)).to.equal("gasless");
  });
  ...
  it("rejects replayed nonce", async () => {
    ...
    const sig = await sign(owner, domain, "CommitCallsignIntent", { owner: owner.address, commitment, ...w });
  ...
  it("rejects a signature from someone other than the owner", async () => {
    ...
    const badSig = await sign(target, domain, "CommitCallsignIntent", { owner: owner.address, commitment, ...w }); // target signs, claims owner
    await expect(
  ...
  it("relayer submits release and repoint intents (pin the callsignHash)", async () => {
    const { owner, relayer, target, reg, domain } = await fixture();
    ...
    await reg.connect(relayer).commitWithSig(owner.address, commitment, w.nonce, w.validAfter, w.validBefore,
      await sign(owner, domain, "CommitCallsignIntent", { owner: owner.address, commitment, ...w }));
    await time.increase(120);
    ...
    await reg.connect(relayer).registerWithSig(owner.address, "lifecycle", SALT, w.nonce, w.validAfter, w.validBefore,
      await sign(owner, domain, "RegisterCallsignIntent", { owner: owner.address, callsign: "lifecycle", salt: SALT, ...w }));
    ...
    await reg.connect(relayer).requestRepointWithSig(owner.address, h, target.address, w.nonce, w.validAfter, w.validBefore,
      await sign(owner, domain, "RequestRepointIntent", { owner: owner.address, callsignHash: h, newOwner: target.address, ...w }));
    expect((await reg.getCallsignInfoByHash(h)).pendingOwner).to.equal(target.address);
  });
});
