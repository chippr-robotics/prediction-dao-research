 ...
 * 
 * ════════════════════════════════════════════════════════════════════════════════════
 * UNDERSTANDING THIS CONTRACT: A "WILL IT RAIN TOMORROW?" STORY
 * ════════════════════════════════════════════════════════════════════════════════════
 * 
 * Imagine you want to create a prediction market for "Will it rain tomorrow in New York?"
 * This contract helps you set up a market where people can bet on the outcome using tokens.
 * 
 * 🌧️ THE RAIN PREDICTION MARKET EXAMPLE:
 * 
 * 1️⃣ CREATING THE MARKET (deployMarketPair function):
 *    - The market creator calls deployMarketPair() with:
 *      • proposalId: A unique ID for this prediction (e.g., "NY_RAIN_JAN6_2026")
 *      • collateralToken: The currency used for betting (e.g., USDC stablecoin)
 *      • liquidityAmount: Initial market liquidity (e.g., $10,000 USDC)
 *      • betType: BetType.YesNo (for "YES it will rain" vs "NO it won't")
 * 
 *    What happens behind the scenes:
 *    - The contract creates a "condition" in CTF1155 (Conditional Token Framework)
 ...
 * 
 * 2️⃣ BUYING PREDICTIONS (buyTokens function):
 *    Alice believes it will rain, so she wants to buy RAIN tokens:
 *    - Alice calls buyTokens(marketId, buyPass=true, amount=100 USDC)
 *    - Her 100 USDC collateral is sent to the contract
 *    - The contract "splits" her collateral into BOTH outcomes using CTF1155:
 *      • 100 RAIN tokens (she gets these!)
 *      • 100 NO-RAIN tokens (contract keeps these for later)
 *    - Alice receives 100 RAIN tokens representing her prediction
 * 
 *    Bob thinks it won't rain, so he buys NO-RAIN tokens:
 *    - Bob calls buyTokens(marketId, buyPass=false, amount=50 USDC)
 *    - His 50 USDC is split into 50 RAIN + 50 NO-RAIN tokens
 *    - Bob receives 50 NO-RAIN tokens for his prediction
 * 
 *    Key insight: Every 1 USDC of collateral creates BOTH 1 RAIN and 1 NO-RAIN token.
 *    This maintains the guarantee that exactly one outcome will win!
 * 
 * 3️⃣ TRADING POSITIONS (sellTokens function):
 *    Before the market closes, Charlie changes his mind:
 *    - Charlie originally bought 75 RAIN tokens but now thinks it won't rain
 *    - He calls sellTokens(marketId, sellPass=true, tokenAmount=75)
 *    - To sell, the contract needs the OPPOSITE position to "merge" back to collateral
 *    - If the contract has 75 NO-RAIN tokens available:
 ...
 * 
 * 4️⃣ MARKET RESOLUTION (resolveMarket function):
 *    Tomorrow arrives and the oracle checks: Did it rain?
 *    
 *    Scenario A - It rained! ☔
 *    - Oracle calls resolveMarket(marketId, passValue=1, failValue=0)
 ...
 *    - Oracle calls resolveMarket(marketId, passValue=0, failValue=1)
 *    - NO-RAIN tokens win, RAIN tokens lose
 * 
 * 5️⃣ CLAIMING WINNINGS (via CTF1155.redeemPositions):
 *    After resolution, winners redeem their tokens for collateral:
 *    
 *    If it rained:
 *    - Alice has 100 RAIN tokens → She calls CTF1155.redeemPositions() → Gets 100 USDC! 🎉
 *    - Bob has 50 NO-RAIN tokens → He redeems → Gets 0 USDC (he lost) 😢
 * 
 *    If it didn't rain:
 ...
 * 
 * • BINARY OUTCOMES: Every market has exactly 2 possible outcomes (rain/no-rain, yes/no, etc.)
 * 
 * • CONDITIONAL TOKENS (CTF1155): Think of these as "maybe money" - they're only worth
 *   something IF their condition is true. RAIN tokens are only valuable if it actually rains.
 * 
 * • COLLATERAL: Real money (like USDC) that backs the prediction tokens. When you buy
 *   prediction tokens, your collateral is locked up until the outcome is known.
 * 
 * • POSITION SPLITTING: 1 USDC always creates 1 RAIN + 1 NO-RAIN token. This ensures
 *   the math always works: since exactly one outcome will happen, the total value equals
 *   the collateral locked (1 USDC).
 * 
 * • POSITION MERGING: Having both RAIN and NO-RAIN tokens? You can merge them back to
 *   get your collateral, since you're guaranteed to win either way (you have both sides!).
 * 
 * • MARKET RESOLUTION: An oracle (trusted data source) determines which outcome actually
 *   happened. This updates the CTF1155 contract so winners can claim their collateral.
 * 
 * 💡 REAL-WORLD USE CASES:
 * - Weather predictions (will it rain, will it snow, temperature above/below)
 * - Sports outcomes (will Team A win, will score be over/under X)
 * - Governance proposals (will proposal pass, will it fail)
 * - Economic indicators (will GDP be above/below forecast)
 * - Event predictions (will product launch on time, will event happen)
 * 
 * ════════════════════════════════════════════════════════════════════════════════════
 * 
 * TRADING INTEGRATION:
